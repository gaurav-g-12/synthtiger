...
class Multiline(templates.Template):
    ...
    def generate(self):
        texts = [self.corpus.data(self.corpus.sample()) for _ in range(10)]
        fonts = [self.font.sample() for _ in range(self.count)]
        color = self.color.data(self.color.sample())
        fg_style = [self.style.sample() for _ in range(self.count)]
        boxes_all_word = []
        ...
        for word, font in zip(texts,fonts):
            ...
            # self.style.apply([text_layer, *char_layers], fg_style[0])
        ...
        number_of_words = random.randint(2, 12)
        random_offset = random.randint(5, 25)
        for word, font in zip(texts, fonts):
            ...
            # self.style.apply([char_layers], fg_style[0])
            ...
            box_coordinate = [int(fx-5+random_offset), int(fy+random_offset), int(fw+7), int(fh+1)]
